...
module.exports.generateFonts = function (options) {
    ...
    files.sort(function (a, b) {
        // 按名称排列
        if (order == "name") {
            return parseInt(a.match(/[\\|\/]icon[\\|\/](\d+).*\.svg$/i)[1]) - parseInt(b.match(/[\\|\/]icon[\\|\/](\d+).*\.svg$/i)[1]);
        } else {
    ...
    function getNextCodepoint() {
        while (_.contains(o.codepoints, currentCodepoint)) {
            currentCodepoint = (currentCodepoint.toString(16)).substring(0, 1) + (parseInt(currentCodepoint.toString(16).substr(1)) + 1);
            currentCodepoint = parseInt(currentCodepoint, 16);
        }
